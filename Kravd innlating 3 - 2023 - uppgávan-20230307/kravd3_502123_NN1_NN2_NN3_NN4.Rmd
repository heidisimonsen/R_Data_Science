...
ggplot(data = na.omit(ferdslu_ohapp), mapping = aes(x = `Ferðsluóhapp tilsamans`, y = `ár`)) +
  ...
  facet_wrap(~stað)
```
    Vit kunnu observera outliers í hvørjun boxplotti á kommunalvegi frá 1995 til 2012 (uttan 2009), 
    men harafturímótir kunnu vit bert obsavera ein á landavegi í ár 2006.
...
```
   ...
   hetta kemur av at flestu outliers vóru frá einari sýsla (Streymoyar sýsla), men nú har vit hava deilt okkara 
   data eftur sýslum eru hesi data ikki longur presenteraði sum bert outliers. 
...
```
    ...
    har munurin á ferðsluóhapp ávíkavist á kommunalvegi og landavegi er stórst, 
    við flest ferðsluóhappum á kommunalvegi. 
    Um vit hyggja at Eysturoyar sýsla sast at antal av ferðsluóhapp ávíkavist á kommunalvegi og landavegi,
    eru meria lík, og at summi ár eru flest fersluóhap á landaveginum(1996,2002,2007,2008,2011) og at Sabdoya sýsla 
    Meðan Sandoyar, Suðuroyar og Vága sýslurnar síggja vit at flestu áruni er ein 
...
``` 
    ...
    Man kundi vænta, at grundin til at probaliteturin fyri at fáa skaða á landsvegi er størri, vegn at fólk koyra skjótari á landsvegi í mun til á komunalvegi.
...
> 4f) ger eina pipe av `ferdslu_ohapp` har tú ikki tekur árið `2009` við, síðani bólkar uppá `stað`, telur antal datapunkt `n` og roknar tal av óhappum `n_ohapp`, tal av skaddum `n_skadd` og roknar sannlíkindini `p_skadd` fyri fólkaskaða. Goym úrslitið í `ohapp_yvirlit` og les hana út (tip: tú fært eina `2 x 5` tibble).
