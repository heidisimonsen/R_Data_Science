...
ggplot(p_samadag, aes(x = n_lesandi, y = p_sdag)) +
  ...
  scale_color_manual(name = "Samanburð", values = c("Simulera" = "black", "Teoretisk" = "red"))+
...
geom_hline(aes(yintercept=0.5, linetype = "0.5"), color = "blue") +
  ...
  scale_linetype_manual(name ="Sannlíkindini", values = c('dashed','dashed','dashed'), 
                        guide = guide_legend(override.aes = list(color = c("blue", "green", "darkorange"))))+
  geom_vline(aes(xintercept =x1), color = "blue") +
  geom_vline(aes(xintercept =x2),color = "green") +
  geom_vline(aes(xintercept =x3),color = "darkorange") 
...
```{r}
ggplot(data = na.omit(ferdslu_ohapp), aes(x = `Skadd fólk`/`Ferðsluóhapp tilsamans`, y = `stað`)) + 
  geom_boxplot() +
  geom_violin(alpha = 0.25) +
  labs(x = "Skadd fólk/Ferðsluóhapp tilsamans", y = "stað")

ggplot(data = na.omit(ferdslu_ohapp), aes(x = `Skadd fólk`/`Ferðsluóhapp tilsamans`, y = "Í Føroyum")) +
  geom_boxplot() +
  geom_violin(alpha = 0.25) +
  labs(x = "Skadd fólk/Ferðsluóhapp tilsamans", y = "Í Føroyum")
...
```{r}
t.test(ohapp_yvirlit$p_skadd, conf.level = .95)$conf.int
...
```{r}
`kommunalvegi` <- ohapp_yvirlit %>% filter(`stað` == "Á kommunalvegi")
prop.test(x = 10, n = `kommunalvegi`$n, p = `kommunalvegi`$p_skadd)
`landsvegi` <- ohapp_yvirlit %>% filter(`stað` == "Á landsvegi")
prop.test(x = 10, n = `landsvegi`$n, p = `landsvegi`$p_skadd)
